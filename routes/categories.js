...
router.post('/', async (req, res) => {
  try {
    ...
    const category = new Category({ name, slug, description, image, icon, subcategories: [] });
    await category.save();
...
// Get feature template (specific route)
router.get('/:categoryId/template/:subcategoryId?', async (req, res) => {
  try {
    const { categoryId, subcategoryId } = req.params;
    if (!mongoose.Types.ObjectId.isValid(categoryId)) {
      return res.status(400).json({ featureFields: [], specFields: [] });
    }
    ...
    if (!template) {
      return res.status(404).json({ featureFields: [], specFields: [] });
    }
...
// Create/Update feature template
...
router.post('/:categoryId/subcategories/:subcategoryId/brands', async (req, res) => {
  try {
    ...
    if (!brand || !brand.trim()) return res.status(400).json({ error: 'Brand name is required' });
    ...
    if (!subcategory.brands) subcategory.brands = [];
    subcategory.brands.push(brand.trim());
    await category.save();

    res.status(201).json({ message: 'Brand added successfully', brands: subcategory.brands });
  } catch (error) {
...
router.delete('/:categoryId/subcategories/:subcategoryId/brands/:brand', async (req, res) => {
  try {
    ...
    res.json({ message: 'Brand deleted successfully', brands: subcategory.brands });
  } catch (error) {
